...
app.get("/",function(req,res){
    ...
    Post.find({},function(err,posts){
      if(!err){
        res.render("home",{
          StrtContent: homeStartingContent,
          objectBody: posts       
        });  
      }
    });
...
app.get("/about",function(req,res){
    res.render("about",{
      StrtContent: aboutContent       
    });
});

app.get("/contact",function(req,res){
  res.render("contact",{
    StrtContent: contactContent       
  });
});
...
app.get("/posts/:postId", function(req,res){
    ...
    Post.find({_id:req.params.postId},function(err,posts){
      if(!err){
        res.render("post",{
          content: posts[0].postBody,
          heading: posts[0].postTitle
        });
        console.log(posts[0].postBody +" "+ posts[0].postTitle);
      }
    });
    ...
    //        heading: post.title
    //      });
    //      console.log(post.postBody +" "+ post.title);
    //   }else{
